...
annot_filename = 'Naveen_OpenPalm_Close.txt'
video_filename = 'Naveen_OpenPalm_Close_rgb.avi'
write_folder = '.\\Test'

with open(annot_filename) as fid:
	annot = fid.readlines()
	annot = map(int, annot)
...
while True and gest_count < annot.shape[0]:
	if cap.isOpened():
		...
		if annot[gest_count, 0] == image_count:
			write_path = os.path.join(write_folder, annot_filename.split('.')[0]+'_'+str(gest_count)+'.avi')
